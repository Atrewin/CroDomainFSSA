...
def addGraphFeature2ReviwsJson(model, reviewsJsons, all_seeds, maps, relation_map, unique_nodes_mapping):
    """
    ...
    for j in tqdm(range(len(x)), position=0, leave=False):
        ...
        try:# concept_graphs[item] 这里不应该找从大图找到的子图，应该要自己构造，应该是opinioncept + conceptNet
            rawConceptTriples = getReviewConceptTriples(x[j])#上面的去重操作失效了, 但是内部所如果找不到，就过滤掉的操作
        ...
        for url in urlList:
            ...
            if "unlabeled" not in url:
                ...
                for mode in ["pos", "neg"]:
                    ...
                    reivewJsonList = addGraphFeature2ReviwsJson(model, reivewJsonList, all_seeds, maps, relation_map, unique_nodes_mapping)
                ...
                json_data = addGraphFeature2ReviwsJson(model, json_data, all_seeds, maps, relation_map, unique_nodes_mapping)
                json_util.dump(json_data, url)
