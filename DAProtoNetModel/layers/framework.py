    ...
    def train(self,
        ...
        for it in range(start_iter, start_iter + train_iter):
            ...
            if self.adv and it > opt.start_train_adv:  # < 优先对抗策略 >后对抗策略
                ...
                features_ori = model.sentence_encoder.module.in_encoder(support)  # (B*2*K, hidden_size)# 这里选这从新forward（没必要学KIONG那样增加了强耦合，只为了减少一次forward）
                ...
                if it > opt.start_train_dis :
                    sum_loss = (loss_dis + sen_loss_dis*20)/21
                else:
                ...
                # #TODO query sentiment
                features_ori = model.sentence_encoder.module.in_encoder(query)
                query_total = features_ori.size(0)
                sen_logits = self.sen_d(features_ori)
                _, sen_pred = sen_logits.max(-1)
                sen_right_query = float((sen_pred == label).long().sum()) / float(query_total)


                iter_loss_dis += self.item(loss_dis.data)
                iter_right_dis += right_dis
                iter_sen_right_dis += sen_right_dis
                iter_sen_right_query += sen_right_query
                iter_dis += 1
            ...
            if self.adv:
                sys.stdout.write(
                    'step: {0:4} | loss: {1:2.6f}, accuracy: {2:3.2f}%, dis_loss: {3:2.6f}, dis_acc: {4:2.6f}, support_sen_acc: {5:2.6f}, query_sen_acc: {5:2.6f}'
                    .format(it + 1, iter_loss / iter_proto,
                            ...
                            100 * iter_sen_right_query / iter_dis,) + '\r')
            else:
                sys.stdout.write(
