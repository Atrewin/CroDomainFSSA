...
class FewDataset(data.Dataset):
    ...
    def __init__(self, name, encoder, N, K, Q, na_rate, root):
        self.root = root
        path = path = getDatesetFilePath(root, name)
    ...
    def __getitem__(self, index):
        ...
        for i, class_name in enumerate(target_classes):
            indices = np.random.choice(
                    list(range(int(len(self.json_data[class_name])*0.5))),
                    self.K + self.Q, False)
            count = 0
            for j in indices:
...
class EvalDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return support_set["word"], support_label
...
def collate_fn(data):
    ...
    support_sets, query_sets, query_labels,  support_labels= zip(*data)
    ...
    return batch_support, batch_query, batch_label, batch_support_labels

def collate_rel_fn(data):
    batch_support = {'word': []}
    batch_query = {'word': []}
    batch_label = []
    batch_support_labels = []
    support_sets, support_labels, query_sets, query_labels= zip(*data)
    for i in range(len(support_sets)):# i = batch
        for k in support_sets[i]:# k = ["word", "graphFeature"]
            batch_support[k] += support_sets[i][k]
        for k in query_sets[i]:
    ...
    batch_label = torch.tensor(batch_label)
    batch_support_labels = torch.tensor(batch_support_labels)
    return batch_support, batch_support_labels, batch_query, batch_label
...
def collate_eval_fn(data):
    ...
    support_sets, support_lsbels= zip(*data)
    for i in range(len(support_sets)):# i = batch
        for k in support_sets[i]:# k = ["word", "graphFeature"]
            batch_support[k] += support_sets[i][k]
    ...
    return batch_support, batch_support_lsbels

def get_loader(name, encoder, N, K, Q, batch_size, 
        num_workers=1, collate_rel_fn=collate_rel_fn, na_rate=0, root='./data/domain_data/processed_data', opt=None):
    dataset = FewDataset(name, encoder, N, K, Q, na_rate, root)#已经ID化 @jinhui 改变了FewRelDataset
